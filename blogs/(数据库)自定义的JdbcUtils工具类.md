>OracleJDBC驱动包是需要Oracle官方授权才能被下载，maven官方仓库里找不到，需要将对应的jar手动发布到本地仓库，具体做法参考我师兄的博客：http://www.jianshu.com/p/0f636d4b684a
...
//            Class.forName("com.mysql.jdbc.Driver");//加载Mysql数据库驱动
            ...
            while (resultSet.next()){
                ...
                for (int i = 1; i <= columnCount; i++) {//请注意，列序号从 DeepCloneUtils 开始，而不是从 deep_clone_utils 开始
                    rowData.put(metaData.getColumnName(i).toUpperCase(), resultSet.getObject(i));//获取键名及值
                }
                list.add(rowData);
            }
        } catch (Exception e){
